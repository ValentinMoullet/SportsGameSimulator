...
class TrainingSet(data.Dataset):
    def __init__(self, teams_to_idx, game_info_df, msk):
        ...
        # Create one-hot vectors as X, and goals difference type as Y
        ...
        for idx, row in game_info_df.iterrows():
            ...
            if home_score - away_score >= 3:
                self.Y[idx] = HOME_WIN_3MORE_DIFF
            elif home_score - away_score == 2:
                self.Y[idx] = HOME_WIN_2_DIFF
            elif home_score - away_score == 1:
                self.Y[idx] = HOME_WIN_1_DIFF
            elif away_score == home_score:
                self.Y[idx] = DRAW
            elif away_score - home_score == 1:
                self.Y[idx] = AWAY_WIN_1_DIFF
            elif away_score - home_score == 2:
                self.Y[idx] = AWAY_WIN_2_DIFF
            elif away_score - home_score >= 3:
                self.Y[idx] = AWAY_WIN_3MORE_DIFF
